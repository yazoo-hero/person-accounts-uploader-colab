...
class BalanceCalculator:
    ...
    def calculate_correct_values(
        ...
        try:
            ...
            absence_rules = self.rules.get("absence_rules", {}).get(
                absence_key,
                self.rules.get("absence_rules", {}).get("default", {})
            )
    ...
    def _calculate_standard_values(
        ...
        beginning_balance = float(row.get("Beginning Year Balance", 
                                        defaults.get("beginning_year_balance", 0)))
        accrued_this_year = float(row.get("Accrued this year",
                                        defaults.get("accrued_this_year", 0)))
        ...
        if hire_date and rules.get("prorate_first_year", False):
            accrued_this_year = self._prorate_accrual(
                hire_date,
                accrued_this_year,
                float(rules.get("accrual_rate", 0))
            )
        ...
        correct_accrual = self._apply_limits(
            accrued_this_year,
            0,
            global_settings.get("max_accrual", 999)
        )
