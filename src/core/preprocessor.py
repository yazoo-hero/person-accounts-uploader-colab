...
class DataPreprocessor:
    ...
    def merge_workday_with_people(
        ...
        required_people_cols = ["WiserId", "Latest Headcount Hire Date", "Latest Headcount Primary Work Email"]
        ...
        logger.info(f"Merged {len(workday_df)} Workday records with {len(people_df)} people records")
    ...
    def merge_calabrio_with_person(
        ...
        logger.info(f"Merged {len(calabrio_df)} Calabrio records with {len(person_df)} person records")
    ...
    def standardize_column_types(df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Numeric columns
        numeric_cols = [
            "BalanceIn", "Accrued", "Extra", "Units Approved",
